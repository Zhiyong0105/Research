...
def inti(mx,my):
    ...
    vec_mask_list1 = []
    for i in range(mx+1):
        vec_mask_list.append("v{}".format(i)) #col
        vec_mask_list1.append("v{}".format(i)) # var
    # print(vec_mask_list)
    count1 =1
    for i in range(mx+1):
        print("  {}_vec = _mm512_maskz_loadu_pd(mask, {});".format(vec_mask_list[i],vec_mask_list[i]))
    print('\n')
    for j in range(my):
        if j>0 :
            print("  _mm512_mask_storeu_pd({0}, mask, {0}_vec);".format(vec_mask_list[-1]))
            vec_mask_list.pop()
            vec_mask_list = ["v{}".format(mx+j)]+ vec_mask_list[0:mx]
            print("  {}_vec = _mm512_maskz_loadu_pd(mask, v{});".format(vec_mask_list1[0],mx+j))
            
        for i in range(mx):
            a = vec_mask_list1[i]
            b = vec_mask_list1[i+1]
            print("  tmp_vec = {}_vec;".format(a))
            print("  {0}_vec = _mm512_add_pd(_mm512_mul_pd(g{2}, tmp_vec), _mm512_mul_pd(s{2}, {1}_vec));".format(a,b,count1))
            print("  {0}_vec = _mm512_sub_pd(_mm512_mul_pd(g{1}, {0}_vec), _mm512_mul_pd(s{1}, tmp_vec));".format(b,count1))
            count1 += 1
            print('\n')
        if j<my-1:
            vec_mask_list1 = [vec_mask_list1[-1]] + vec_mask_list1[0:mx]
        
    for i in range(len(vec_mask_list)):
        print("  _mm512_mask_storeu_pd({}, mask, {}_vec);".format(vec_mask_list[i],vec_mask_list1[i]))
