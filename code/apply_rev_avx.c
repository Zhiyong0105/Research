...
v10 = _mm256_loadu_pd(&V[16 + 0]);
v11 = _mm256_loadu_pd(&V[16 + 4]);
v12 = _mm256_loadu_pd(&V[16 + 8]);
v13 = _mm256_loadu_pd(&V[16 + 12]);
    gamma = _mm256_broadcast_sd(&G[2 * 0 + k * ldg]);
    sigma = _mm256_broadcast_sd(&G[2 * 0 + k * ldg + 1]);
 tmp = v00;
 v00 = _mm256_add_pd(_mm256_mul_pd(gamma, tmp), _mm256_mul_pd(sigma, v10));
...
for (int g = 1; g < n - 1; g++)
{
v20 = _mm256_loadu_pd(&V[(g + 1) * 16 + 0]);
v21 = _mm256_loadu_pd(&V[(g + 1) * 16 + 4]);
v22 = _mm256_loadu_pd(&V[(g + 1) * 16 + 8]);
v23 = _mm256_loadu_pd(&V[(g + 1) * 16 + 12]);
gamma = _mm256_broadcast_sd(&G[2 * g + k * ldg]);
sigma = _mm256_broadcast_sd(&G[2 * g + k * ldg + 1]);
tmp = v10;
 v10 = _mm256_add_pd(_mm256_mul_pd(gamma, tmp), _mm256_mul_pd(sigma, v20));
...
tmp = v03;
 v03 = _mm256_add_pd(_mm256_mul_pd(gamma, tmp), _mm256_mul_pd(sigma, v13));
 v13 = _mm256_sub_pd(_mm256_mul_pd(gamma, v13), _mm256_mul_pd(sigma, tmp));
_mm256_storeu_pd(&V[16 * (g - 1)], v00);
_mm256_storeu_pd(&V[16 * (g - 1) + 4 * 1], v01);
_mm256_storeu_pd(&V[16 * (g - 1) + 4 * 2], v02);
_mm256_storeu_pd(&V[16 * (g - 1) + 4 * 3], v03);
v00=v10;
v01=v11;
v02=v12;
v03=v13;
...
tmp = v03;
 v03 = _mm256_add_pd(_mm256_mul_pd(gamma, tmp), _mm256_mul_pd(sigma, v13));
 v13 = _mm256_sub_pd(_mm256_mul_pd(gamma, v13), _mm256_mul_pd(sigma, tmp));
_mm256_storeu_pd(&V[16 * (n - 2)], v00);
_mm256_storeu_pd(&V[16 * (n - 2)+ 4 * 1], v01);
_mm256_storeu_pd(&V[16 * (n - 2)+ 4 * 2], v02);
_mm256_storeu_pd(&V[16 * (n - 2)+ 4 * 3], v03);
_mm256_storeu_pd(&V[16 * (n - 1)], v10);
_mm256_storeu_pd(&V[16 * (n - 1)+ 4 * 1], v11);
_mm256_storeu_pd(&V[16 * (n - 1)+ 4 * 2], v12);
_mm256_storeu_pd(&V[16 * (n - 1)+ 4 * 3], v13);
}
