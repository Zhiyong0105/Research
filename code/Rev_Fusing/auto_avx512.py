...
def apply_rev_auto_mv_avx512(my, mv):
    ...
    ##########################
def apply_rev_auto_mv_seq_fma(my,mv):
    print("void apply_rev_avx_mv_seq_fma(int k,int m, int n, double *G, double *V,int ldg)")
    print("{")
    ...
    for y in range(my-1):
        for k in range(y+1):
            ...
            for v in range(mv):
                ...
                # print(f" v{g}{v} = _mm512_add_pd(_mm512_mul_pd(gamma, tmp), _mm512_mul_pd(sigma, v{g+1}{v}));")
                # print(f" v{g+1}{v} = _mm512_sub_pd(_mm512_mul_pd(gamma, v{g+1}{v}), _mm512_mul_pd(sigma, tmp));")
                print(f" v{g}{v} = _mm512_fmadd_pd(gamma, tmp, _mm512_mul_pd(sigma, v{g+1}{v}));")
                print(f" v{g+1}{v} = _mm512_fmsub_pd(gamma, v{g+1}{v}, _mm512_mul_pd(sigma, tmp));")
    ...
    for y in range(my):
        ...
        for v in range(mv):
            ...
            # print(f" v{my-y-1}{v} = _mm512_add_pd(_mm512_mul_pd(gamma, tmp), _mm512_mul_pd(sigma, v{my-y}{v}));")
            # print(f" v{my-y}{v} = _mm512_sub_pd(_mm512_mul_pd(gamma, v{my-y}{v}), _mm512_mul_pd(sigma, tmp));")
            print(f" v{my-y-1}{v} = _mm512_fmadd_pd(gamma, tmp, _mm512_mul_pd(sigma, v{my-y}{v}));")
            print(f" v{my-y}{v} = _mm512_fmsub_pd(gamma, v{my-y}{v}, _mm512_mul_pd(sigma, tmp));")
    ...
    for k in range(my-1):
        for g in range(my-k-1):
            ...
            for v in range(mv):
                ...
                # print(f" v{gg}{v} = _mm512_add_pd(_mm512_mul_pd(gamma, tmp), _mm512_mul_pd(sigma, v{gg+1}{v}));")
                # print(f" v{gg+1}{v} = _mm512_sub_pd(_mm512_mul_pd(gamma, v{gg+1}{v}), _mm512_mul_pd(sigma, tmp));")
                print(f" v{gg}{v} = _mm512_fmadd_pd(gamma, tmp, _mm512_mul_pd(sigma, v{gg+1}{v}));")
                print(f" v{gg+1}{v} = _mm512_fmsub_pd(gamma, v{gg+1}{v}, _mm512_mul_pd(sigma, tmp));")
...
apply_rev_auto_mv_seq_fma(int(args[1]), int(args[2]))
