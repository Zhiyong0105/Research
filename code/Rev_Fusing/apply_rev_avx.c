...
{
v30 = _mm512_loadu_pd(&V[(g + 1) * 56 + 0]);
v31 = _mm512_loadu_pd(&V[(g + 1) * 56 + 4]);
v32 = _mm512_loadu_pd(&V[(g + 1) * 56 + 8]);
v33 = _mm512_loadu_pd(&V[(g + 1) * 56 + 12]);
v34 = _mm512_loadu_pd(&V[(g + 1) * 56 + 16]);
v35 = _mm512_loadu_pd(&V[(g + 1) * 56 + 20]);
v36 = _mm512_loadu_pd(&V[(g + 1) * 56 + 24]);
gamma = _mm512_set1_pd(G[2 * g + k * ldg]);
sigma = _mm512_set1_pd(G[2 * g + k * ldg + 1]);
tmp = v20;
 v20 = _mm512_add_pd(_mm512_mul_pd(gamma, tmp), _mm512_mul_pd(sigma, v30));
